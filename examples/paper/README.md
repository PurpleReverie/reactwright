...
```
examples/paper/
├── paper.tsx                       entry: imports the template,
...
once; every section imports `Cite` from that file. TypeScript catches
...
`packages/template-ieee/`. This directory contains zero IEEE
knowledge — only the prose, the data, the citation graph, and the
composition.

## Adapting this example

1. Copy `examples/paper/` to a new directory under `examples/` (or
   anywhere in your own workspace).
...
5. Update the `mockup` script in `package.json` to point at the new
   entry file.
