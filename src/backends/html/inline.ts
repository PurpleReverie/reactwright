...
export function renderLinkNode(node: ResolvedLinkNode): string {
  ...
  return `<a href="${escapeHtml(node.href)}"${titleAttr}${classAttr(node)}>${node.children.map(renderInlineNode).join("")}</a>`;
}
...
export function renderInlineImgNode(node: ResolvedInlineImgNode): string {
  ...
  return `<img data-inline src="${escapeHtml(normalizeImageSrc(node.src))}"${altAttr}${widthAttr}${heightAttr}${classAttr(node)} />`;
}
...
export function renderRefNode(node: ResolvedRefNode): string {
  ...
  return `<a data-node="ref" data-ref-to="${escapeHtml(node.to)}" data-ref-show="${escapeHtml(node.show)}"${classAttrWithBase(node, "reactwright-ref", `reactwright-ref-${node.show}`)} href="${href}"></a>`;
}
...
export function renderFootnoteNode(node: ResolvedFootnoteNode): string {
  ...
  return `<span data-node="footnote"${markerAttr}${classAttrWithBase(node, "reactwright-footnote")}>${inner}</span>`;
}

export function renderSidenoteNode(node: ResolvedSidenoteNode): string {
  const inner = node.children.map(renderInlineNode).join("");
  return `<span data-node="sidenote"${classAttrWithBase(node, "reactwright-sidenote")}>${inner}</span>`;
}

export function renderInlineMathNode(node: ResolvedInlineMathNode): string {
  return `<span data-node="math-inline"${classAttrWithBase(node, "reactwright-math", "reactwright-math-inline")}>${renderTeX(node.src, false)}</span>`;
}
...
export function renderCiteNode(node: ResolvedCiteNode): string {
  ...
  return `<a data-node="cite" data-cite-key="${escapeHtml(node.cite)}"${classAttrWithBase(node, "reactwright-cite")} href="${href}"></a>`;
}

export function renderInlineNode(node: ResolvedInlineNode): string {
  switch (node.kind) {
    case "text":     return renderTextNode(node);
    case "em":       return `<em${classAttr(node)}>${node.children.map(renderInlineNode).join("")}</em>`;
    case "strong":   return `<strong${classAttr(node)}>${node.children.map(renderInlineNode).join("")}</strong>`;
    case "code":     return `<code${classAttr(node)}>${node.children.map(renderTextNode).join("")}</code>`;
    case "link":     return renderLinkNode(node);
    case "br":       return "<br />";
    case "sub":      return `<sub${classAttr(node)}>${node.children.map(renderInlineNode).join("")}</sub>`;
    case "sup":      return `<sup${classAttr(node)}>${node.children.map(renderInlineNode).join("")}</sup>`;
